 ...
 * Load the Jquery and migration files
	...
	*
 */
function theme_fiware_page_init(moodle_page $page) {
    $page->requires->jquery();
	   $page->requires->jquery_plugin('migrate');
				$page->requires->js('/theme/fiware/javascript/theme.js');
}
...
function theme_fiware_process_css($css, $theme) {

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_fiware_set_logo($css, $logo);
    ...
    $css = theme_fiware_set_customcss($css, $customcss);
		
		$css = theme_fiware_set_fontwww($css);
 ...
 */
function theme_fiware_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_fiware_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;

    if (empty($theme)) {
        $theme = theme_config::load('fiware');
    }
 ...
 * @param string $filename
	* @return string
 */
function theme_fiware_serve_css($filename) {
    global $CFG;
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . '/fiware/style/';
    } else {
        $thestylepath = $CFG->dirroot . '/theme/fiware/style/';
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_fiware_send_unmodified($lastmodified, $etagfile);
    }
    theme_fiware_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}

// Set browser cache used in php header
function theme_fiware_send_unmodified($lastmodified, $etag) {
...
// Cached css
function theme_fiware_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 */
function theme_fiware_set_customcss($css, $customcss) {
 ...
 */
function theme_fiware_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
 ...
 * @param $css
	* @return string
 */
function theme_fiware_set_fontwww($css) {
    global $CFG, $PAGE;
    ...
    $theme = theme_config::load('fiware');
   	...
   	$css = str_replace($tag, $themewww.'/fiware/fonts/', $css);
...
function theme_fiware_render_slideimg($p,$sliname)
{
				global $PAGE, $OUTPUT;
					
    $nos = theme_fiware_get_setting('numberofslides'); 
    $i = $p%3;
    $slideimage = $OUTPUT->pix_url('home/slide'.$i, 'theme');
				
				// Get slide image or fallback to default
    if (theme_fiware_get_setting($sliname)) {
...
function theme_fiware_get_setting($setting, $format = false) {
    global $CFG;

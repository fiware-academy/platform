...
class theme_fiware_core_course_renderer extends core_course_renderer {
	public function new_courses() {
		/* New Courses */
		global $CFG, $OUTPUT;
		$new_course = get_string ( 'newcourses', 'theme_fiware' );
	...
	private function get_random_image() {
		$dire = "theme/fiware/images/no-image/";
	...
	protected function render_custom_menu(custom_menu $menu) {
		...
		$branch = $menu->add ( get_string ( 'courses', 'theme_fiware' ), null, null, 10000 );
		...
		*/
		...
		if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
			$url = new moodle_url ( '/course/index.php', 
						...
						$categoryname ,array ('title' => get_string ( 'browsetocategory', 'theme_fiware'  )));
		...
		if($coursescount>0){
			if($coursescount>1){
				$coursescount .= ' '.get_string ( 'courses', 'theme_fiware'  );
			}else{
				$coursescount .= ' '.get_string ( 'course', 'theme_fiware'  );
			}
			$coursescount = html_writer::tag ( 'span', ' (' . $coursescount . ')', array (
					'title' => get_string ( 'numberofcourses', 'theme_fiware' ),
	 ...
	 * @return string
	 */
	public function fiware_course_section_cm_name(cm_info $mod, $displayoptions = array()) {
		global $CFG;
		$output = '';
